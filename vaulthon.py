...
Shout out to the original AES-128 implementation by Bo Zhu, and BoppreH for the github example repository
AES in Pure Python: https://github.com/boppreh/aes/blob/master/aes.py
...
if __name__ == '__main__':
    ...
    # Load config.json
    if len(sys.argv) < 1:
        print("Usage: ./vaulthon.py <key>")
        exit(0)
    key = sys.argv[1]
    try:
        with open("config.json", "r") as config_handle:
            file_list: list[str] = load(config_handle)
            for file in file_list:
                encrypted_file = file+ENCRYPTED_SUFFIX
                # Create encrypted file from original files
                if not exists(file):
                    if not exists(encrypted_file):
                        print(f"{file} and {encrypted_file} doesn't exist.")
                        exit(0)
                    with open(encrypted_file, "rb+") as encrypted_file_handle:
                        encrypted_bytes = encrypted_file_handle.read()
                        original_bytes = decrypt(key, encrypted_bytes)
                        lines = original_bytes.decode()
                        with open(file, "w+") as file_handle:
                            file_handle.writelines(lines.split("\n"))
                with open(file, "r") as file_handle:
                    lines = "\n".join([line.rstrip() for line in file_handle])
                    with open(encrypted_file, "wb+") as encrypted_file_handle:
                        encrypted_file_handle.write(encrypt(key, lines))
                        encrypted_file_handle.flush()
                        print(f"Write {encrypted_file}.")
